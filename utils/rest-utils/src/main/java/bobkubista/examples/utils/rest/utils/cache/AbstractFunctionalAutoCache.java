 ...
 */
public abstract class AbstractFunctionalAutoCache<V extends AbstractGenericFunctionalIdentifiableDomainObject, COL extends AbstractGenericDomainObjectCollection<V>>
		extends AbstractIdentifiableAsyncCaffeineCache<V, COL> {
	...
	private final AsyncLoadingCache<String, Long> functionalToIdentifiercache = Caffeine.newBuilder()
	...
	public CompletableFuture<V> get(final String functionalId) throws InterruptedException, ExecutionException {
		return this.get(
				this.functionalToIdentifiercache.get(functionalId, this.getFunctionalService()::getIdByFunctionalId)
						.get());
	}

	@Override
	public V reload(final Long key, final V oldValue) throws Exception {
	 ...
	 */
	protected abstract FunctionalIdentifiableService<V, COL> getFunctionalService();

	@Override
	protected final IdentifiableService<V, COL> getIdentifiableService() {
