...
public class MockFacade extends AbstractGenericActiveFacade<MockDomain, Long, MockEntity, MockDomainCollection> {
	...
	protected ActiveEntityService<MockEntity, Long> getService() {
		...
		Mockito.when(mock.getAll(Collections.singletonList("id"), 0, 100)).thenReturn(Collections.singletonList(this.buildMockEntity()));
		Mockito.when(mock.getAllActive(new ArrayList<>(), 0, 100)).thenReturn(Collections.singletonList(this.buildMockEntity()));
