...
public class AbstractEntityToDomainConverterTest {
	...
	public void testConvertToDomainObjectCollectionOfEONull() {
		...
		Assert.assertEquals(0, result.getDomainCollection()
				.size());
	}
	...
	public void testConvertToEntityDMO() {
		...
		Mockito.verify(this.converter.getMockService())
				.getById(1L);
	}

	@Test
	public void testConvertToEntityDMONewEntity() {
		final MockDomain domain = this.buildMockDomain();
		domain.setId(2L);
		...
		Mockito.verify(this.converter.getMockService(), Mockito.never())
				.getById(2L);
	}
	...
	public void testConvertToEntityDomainObjectCollectionOfDMO() {
		...
		domainObjects.getDomainCollection()
				.add(this.buildMockDomain());

		final Collection<MockEntity> result = this.converter.convertToEntity(domainObjects);

		Assert.assertNotNull(result);
		Assert.assertFalse(result.isEmpty());
		Assert.assertEquals(domainObjects.getDomainCollection()
				.size(), result.size());
	}
